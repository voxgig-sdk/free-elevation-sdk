...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = FreeElevationOmni.make_runner(TEST_JSON_FILE, FreeElevationSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    FreeElevationError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : FreeElevationOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = FreeElevationOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
