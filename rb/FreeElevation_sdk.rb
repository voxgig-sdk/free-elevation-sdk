...
require_relative 'FreeElevation_types'
...
class FreeElevationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = FreeElevationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = FreeElevationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, FreeElevationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    FreeElevationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = FreeElevationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = FreeElevationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = FreeElevationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = FreeElevationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = FreeElevationError.new(
        "graphql_error", "FreeElevationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = FreeElevationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
