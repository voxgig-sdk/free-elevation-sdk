...
class FreeElevationTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeElevationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeElevationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeElevationTestRunner.getenv("FREE_ELEVATION_TEST_LIVE")
        override = FreeElevationTestRunner.getenv("FREE_ELEVATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeElevationTestRunner.getenv(key)
        ...
        explain = FreeElevationTestRunner.getenv("FREE_ELEVATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FreeElevationTestRunner._test_control is not None:
            return FreeElevationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeElevationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeElevationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeElevationTestRunner.load_test_control()
...
def load_env_local():
    FreeElevationTestRunner.load_env_local()


def env_override(m):
    return FreeElevationTestRunner.env_override(m)


def entity_data(v):
    return FreeElevationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return FreeElevationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeElevationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeElevationTestRunner.load_test_control()


def live_client_options():
    return FreeElevationTestRunner.live_client_options()


def live_delay_ms():
    return FreeElevationTestRunner.live_delay_ms()
