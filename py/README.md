...
## Install
```bash
pip install voxgig-sdk-free-elevation
```
...
```python
result, err = client.Elevation().list()
if err:
    raise Exception(err)
...
```python
result, err = client.Elevation().load({"id": "example_id"})
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.FreeElevation().load({"id": "test01"})
