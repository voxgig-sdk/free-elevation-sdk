...
func TestElevationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Elevation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Elevation(nil).Stream("list", nil, nil) {
...
func elevationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("FREE_ELEVATION_TEST_ELEVATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["FREE_ELEVATION_TEST_ELEVATION_ENTID"])
