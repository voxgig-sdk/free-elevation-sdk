...
import (
	...
	omni "github.com/voxgig-sdk/free-elevation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.FreeElevationError); is {
				out := map[string]any{
					"name":    "FreeElevationError",
...
func omniCtx(arg any, client *sdk.FreeElevationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.FreeElevationSDK); is {
			client = live
