...
// FreeElevation SDK retry feature
...
class FreeElevationRetryFeature extends FreeElevationBaseFeature
{
    ...
    public function init(FreeElevationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (FreeElevationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(FreeElevationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(FreeElevationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
