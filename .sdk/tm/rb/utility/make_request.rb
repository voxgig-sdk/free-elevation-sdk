...
module FreeElevationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeElevationResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeElevationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeElevationResponse.new(fetched)
    else
