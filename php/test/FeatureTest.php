...
// FreeElevation SDK feature test
//
...
class FtCtrl extends FreeElevationControl
{
...
class FtClient
{
    ...
    private FreeElevationUtility $utility;

    public function __construct(FreeElevationUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeElevationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeElevationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeElevationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeElevationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeElevationConfig::make_feature($name);
    ...
    private static function buildUrl(FreeElevationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeElevationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeElevationResponse($fetched);
        }
    ...
    private function fail(FreeElevationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeElevationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
