...
The PHP SDK for the FreeElevation API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Elevation(null)->list(null, null);
...
```php
[$result, $err] = $client->Elevation(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->FreeElevation(null)->load(
    ["id" => "test01"], null
);
