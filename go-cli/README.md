...
boru-driven command-line client **and** interactive REPL for the FreeElevation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-elevation-cli)
make build

# 2. See usage (words, entities, env vars)
./free-elevation-cli --help

# 3. Provide credentials once, via the environment
export FREE_ELEVATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./free-elevation-cli list elevation
./free-elevation-cli load 1 elevation            # {id:1} shorthand
./free-elevation-cli load '{id:1}' elevation       # explicit match map

# 5. Override the API base URL for a single call
FREE_ELEVATION_BASE=https://api.example.com ./free-elevation-cli list elevation

# 6. No arguments -> interactive REPL
./free-elevation-cli
free-elevation> list elevation
free-elevation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-elevation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/free-elevation-cli list elevation
   ```
...
```sh
./free-elevation-cli load 1 elevation          # scalar shorthand for {id:1}
./free-elevation-cli load '{id:1}' elevation     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `free-elevation>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `elevation`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-elevation-cli`. |
...
(`./free-elevation-cli <expr>`) and interactively (the REPL), and expressions compose the
