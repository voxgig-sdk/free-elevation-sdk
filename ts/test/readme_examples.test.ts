...
//     (\`new FreeElevationSDK(...)\` / \`FreeElevationSDK.test(...)\` become
//     \`FreeElevationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'FreeElevationSDK'
...
const TEST_SEED = {"entity":{"elevation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeElevationSDK(...)\` and any
// \`FreeElevationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeElevationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeElevationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
