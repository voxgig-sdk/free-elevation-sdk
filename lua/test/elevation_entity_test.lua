...
describe("ElevationEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Elevation(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Elevation(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set FREE_ELEVATION_TEST_ELEVATION_ENTID JSON to run live")
      return
    end
...
function elevation_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("FREE_ELEVATION_TEST_ELEVATION_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["FREE_ELEVATION_TEST_ELEVATION_ENTID"])
  ...
  return {
    ...
    env = env,
    explain = env["FREE_ELEVATION_TEST_EXPLAIN"] == "TRUE",
    live = live,
