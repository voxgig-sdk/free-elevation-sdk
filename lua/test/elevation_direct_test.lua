...
describe("ElevationDirect", function()
  it("should direct-list-elevation", function()
    local setup = elevation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-elevation", setup.live and "live" or "unit")
  ...
  it("should direct-load-elevation", function()
    local setup = elevation_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-elevation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function elevation_direct_setup(mockres)
