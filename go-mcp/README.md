...
[MCP](https://modelcontextprotocol.io) server exposing the FreeElevation SDK as
two agent tools — `free-elevation_list` and `free-elevation_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-elevation-mcp)
make build

# 2. Provide credentials via the environment
export FREE_ELEVATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-elevation \
  -- /absolute/path/to/free-elevation-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-elevation-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-elevation_list: first page of records
{ "entity": "elevation" }
{ "entity": "elevation", "query": { } }

// free-elevation_load: one record by id
{ "entity": "elevation", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-elevation-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-elevation \
     -- "$PWD"/dist/*/free-elevation-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-elevation_list` and `free-elevation_load` tools now appear
   in new sessions. Ask the agent to *"list elevation using free-elevation"*
   and it calls `free-elevation_list` with `{"entity":"elevation"}`.
...
```sh
export FREE_ELEVATION_APIKEY=sk_live_xxx            # API key
export FREE_ELEVATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `free-elevation_list` tool
...
### Call the `free-elevation_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `free-elevation_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-elevation_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_ELEVATION_APIKEY` | API key sent with every request. |
| `FREE_ELEVATION_BASE` | Optional override of the API base URL. |
...
```sh
./free-elevation-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-elevation_load","arguments":{"entity":"elevation","query":{"id":1}}}}'
```
